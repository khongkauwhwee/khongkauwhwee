...
function com_install() {
  ...
  // set admin email
  $sql="UPDATE #__jcalpro2_config SET value = '$mosConfig_mailfrom' WHERE name='calendar_admin_email'";
  $db->setQuery($sql);
  $db->query();
  ...
  if ($success) {
    if ((JFile::exists( JPATH_ROOT.DS.'administrator'.DS.'components' . DS . 'com_community' . DS . 'admin.community.php'))) {
      ...
      $mainframe->enqueueMessage( 'JCal Pro latest events plugin for Jomsocial not installed (Jomsocial not detected on this site).');
    }
  }
  ...
  if ($success) {
    if ((JFile::exists( JPATH_ROOT.DS.'administrator'.DS.'components' . DS . 'com_community' . DS . 'admin.community.php'))) {
      ...
      $mainframe->enqueueMessage( 'JCal Pro mini-calendar plugin for Jomsocial not installed (Jomsocial not detected on this site).');
    }
  }
