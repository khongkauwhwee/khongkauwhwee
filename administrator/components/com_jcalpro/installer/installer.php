...
function installFromDirectory( $installerClass, $option, $element, $client ) {
  ...
  if (!$userfile) {
    $mainframe->redirect( "index.php?option=$option&element=module", "Please select a directory" );
  }
...
function uploadFile( $filename, $userfile_name, &$msg ) {
  ...
  $baseDir = JPATH_ROOT . DS . 'media';

  if (file_exists( $baseDir )) {
    if (is_writable( $baseDir )) {
      if (move_uploaded_file( $filename, $baseDir . DS .$userfile_name )) {
        if (JPath::setPermissions( $baseDir . DS . $userfile_name )) {
          return true;
        } else {
