...
class JCalProInstaller {
  ...
  function extractArchive() {
    ...
    $base_Dir 		= JPATH_ROOT . DS . 'media' . DS ;
    ...
    if (eregi( '.zip$', $archivename )) {
      ...
      require_once( JPATH_ROOT . DS . 'administrator'. DS . 'includes'. DS . 'pcl'. DS . 'pclerror.lib.php' );
  ...
  function returnTo( $option, $element ) {
    return "index.php?option=$option&task=$element";
  }
  /**
        ...
        if ($adminFiles){
          if (!JFolder::create( $this->componentAdminDir().$newdir )) {
            ...
            return false;
    				  ...
    				  if (!JFolder::create( $this->elementDir().DS . $newdir )) {
    				    $this->setError( 1, 'Failed to create directory "' . ($this->elementDir()) . DS . $newdir . '"' );
    				    return false;
    				  }
    				}
      }
  ...
  function copyFiles( $p_sourcedir, $p_destdir, $p_files, $overwrite=true ) {
    ...
    if (is_array( $p_files ) && count( $p_files ) > 0) {
      foreach($p_files as $_file) {
        $filesource	= JPath::clean( JPath::clean( $p_sourcedir ) . DS . $_file );
        $filedest	= JPath::clean( JPath::clean( $p_destdir ) . DS . $_file );
...
function cleanupInstall( $userfile_name, $resultdir) {
  ...
  if (file_exists( $resultdir )) {
    ...
    JFile::delete( JPATH_ROOT . DS . 'media' . DS . $userfile_name );
  }
}
