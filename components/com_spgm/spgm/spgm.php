<?php
/********************************************************************
Original Author: Sylvain Pajot (http://spgm.sourceforge.net/)
This Fork By   : Chris Gaebler (http://www.lesarbresdesign.info/)
Date           : 03 March 2010
Version        : 2.06
Licence        : GNU/GPL
Description    : This is the original SPGM gallery, modified for inclusion
                 in the package SPGM for Mambo and Joomla!
...
if (!defined('DIR_THEMES'))
	define('DIR_THEMES', 'flavors/');		// themes base directory (relative to this file

//define('DIR_THUMBS', 'thumbnails/');		// if defined, points to the directory
											// where thumbnails reside, relative to the gallery
...
//define('FILE_GAL_CAPTION', 'gal-desc.txt'); // no longer used v2.00
...
//define('FILE_PIC_CAPTIONS', 'pic-desc.txt'); // no longer used v2.00
...
function spgm_BuildLink($text, $cssClass, $anchor, $galId, $pageIdx, $picId, $filters) {
  ...
  if ($galId != '') {
    $url .= PARAM_NAME_GALID.'='.rawurlencode($galId);
  }
  if ($pageIdx != -1) {
  ...
  $link = '<a href="'.$url.'" class="'.$cssClass.'">'.utf8_encode($text).'</a>'; 
...
function spgm_LoadPictureCaptions($strGalleryId) {
  ...
  if ($spgm_cfg['conf']['multiLanguageCaptions'])	// v2.00
	  $strCaptionsFilename = DIR_GAL.$strGalleryId.'/pic-desc_'.LANG_AUTO.'.txt';
  else
	  $strCaptionsFilename = DIR_GAL.$strGalleryId.'/pic-desc.txt';
	...
	if ($spgm_cfg['conf']['exifInfo'][0] == 'ALL')
		...
		foreach ($arrExifData as $key => $value)
			if (!is_array($arrExifData[$key]))
				{
				$cleanValue = ereg_replace("[^A-Za-z0-9]", "", $value);
				$strExifData .= '&lt;b&gt;'.$key.'&lt;/b&gt; '.$cleanValue.'&lt;br /&gt;';
				}
  ...
  for ($i=0; $i<$_max; $i++) {
    ...
    if ($i < ($_max - 1)) {
      ...
      if ( strstr($strFilterFlags, PARAM_VALUE_FILTER_NOTHUMBS) 
        ...
        if ($mixPictureId == '') {
          print utf8_encode($strHtmlGalleryName);
        } else {
    ...
    if ($iPictureNumber < 0 || $iNewPictureNumber < 0)
      ...
      for ($j=0; $j<$iSupportedExtensionNumber; $j++) {
        ...
        if ( spgm_CheckPerms($strGalleryThumbnailFilename) ) {
          $strGalleryThumbnailFilename = implode("/", array_map("rawurlencode", explode("/", $strGalleryThumbnailFilename)));
	  ...
	  if ($spgm_cfg['conf']['multiLanguageCaptions'])	// v2.00
	      $strPathToGalleryCaption = $strPathToGallery.'/'.$strGalleryName.'/gal-desc_'.LANG_AUTO.'.txt';
	  else
	      $strPathToGalleryCaption = $strPathToGallery.'/'.$strGalleryName.'/gal-desc.txt';
...
//------------------------------------------------------------------------------
// New for version 2.00. Determine dimensions for image display
//
function spgm_displayDimensions($strPictureURL,&$iWidth,&$iHeight)
{
global $spgm_cfg;

    $imgSize = getimagesize($strPictureURL);

    $iWidth = $imgSize[0];
    $iHeight = $imgSize[1];
    if ($iHeight < $iWidth)			// Landscape (normal) picture
    	{
		if ($spgm_cfg['conf']['fullPictureWidth'] == ORIGINAL_SIZE)
			...
			if ($spgm_cfg['conf']['fullPictureHeight'] != ORIGINAL_SIZE) 
				...
				$iWidth = intval($imgSize[0]*($spgm_cfg['conf']['fullPictureHeight']/$imgSize[1]));
				}
			}
		else 
			{
			$iWidth = $spgm_cfg['conf']['fullPictureWidth'];
			$iHeight = intval($imgSize[1]*($spgm_cfg['conf']['fullPictureWidth']/$imgSize[0]));
			} 
		}
		...
		if ($spgm_cfg['conf']['fullPictureWidth'] == ORIGINAL_SIZE)
			...
			if ($spgm_cfg['conf']['fullPictureHeight'] != ORIGINAL_SIZE)
				{
				$iHeight = $spgm_cfg['conf']['fullPictureWidth'];
				$iWidth = intval($imgSize[0]*($spgm_cfg['conf']['fullPictureWidth']/$imgSize[1]));
				}
			}
		else 
			{
			$iWidth = $spgm_cfg['conf']['fullPictureHeight'];
			$iHeight = intval($imgSize[1]*($spgm_cfg['conf']['fullPictureHeight']/$imgSize[0]));
			} 
		}
}
...
global $spgm_cfg;
  ...
  if ( spgm_IsPicture($strPictureFilename, $strGalleryId) ) {
    ...
    // EXIF data - from v2.00 don't show the prompt in slideshow mode
    
    if ((!$bSlideshowMode) 
    and (count($spgm_cfg['conf']['exifInfo']) > 0)
    and (extension_loaded('exif')))
		{ 							// ... where available
		print '<tr><td>'."\n";
		$strExifData = spgm_LoadExif($strPictureURL);
		print '[<span onmouseover="return overlib(\''.addslashes($strExifData).'\', CAPTION, \''.$spgm_cfg['locale']['exifHeading'].' '.addslashes($strPictureFilename).'\', STICKY);" onmouseout="return nd();" style="color: #2e408d; font-weight: bold; font-size: 9pt;">Exif</span>]';
		print '</td></tr>'."\n";
		}

    // Load pictures if slideshow mode is enabled - modified at v2.00 to use correct sizes
    ...
    if ($bSlideshowMode)
		...
		for ($i=0; $i<$iPictureNumber; $i++)
			...
			$iWidth = 0;
			$iHeight = 0;
			spgm_displayDimensions($_strPicturePath,$iWidth,$iHeight);
			$_strPicturePath = implode("/", array_map("rawurlencode", explode("/", $_strPicturePath)));
			$_strPictureCaption = '';
			if (isset($spgm_cfg['captions'][$arrPictureFilenames[$i]]))
				$_strPictureCaption = $spgm_cfg['captions'][$arrPictureFilenames[$i]];
			// v2.05 added WEB_SITE to path
			print '  addPicture(\''.WEB_SITE.addslashes($_strPicturePath).'\', \''.addslashes($_strPictureCaption).'\', '.$iWidth.', '.$iHeight.');'."\n";
			}
		print '</script>'."\n";
		}

    // compute image dimensions - modified at v2.00 to correctly resize portrait pictures

	$iWidth = 0;
	$iHeight = 0;
	spgm_displayDimensions($strPictureURL,$iWidth,$iHeight);  
	...
	$strPictureURL = implode("/", array_map("rawurlencode", explode("/", $strPictureURL)));
...
function spgm_DisplayThumbnails($strGalleryId, $arrPictureFilenames, $iPictureId, $iPageIndex, $strFilterFlags) {
  ...
  for ($i = $iPictureOffsetStart; $i < $iPictureOffsetStop; $i++) {
    ...
    $strThumbnailURL = implode("/", array_map("rawurlencode", explode("/", $strThumbnailURL)));
...
// Version 2.03, modification suggested by Frederic Mura for compatibility with PHP4. Also works in PHP5.
// When register globals is 0 or when gallery id is empty 
// (can not be read from the previous process) try to get the data from the URL
...
// Template mode - 2.00 moved this outside the else block above
...
if ($strParamGalleryId == '') 		// if gallery not specified, generate the tree
	spgm_DisplayGalleryHierarchy('', 0, $strParamFilterFlags);
else
	{
	print '  <td>'."\n";
	if ($strParamPictureId == '')	// if we've got a gallery but no picture, display thumbnails
...
// display the link to SPGM website - made optional at v2.00

if ($spgm_cfg['conf']['spgmLink'])
	{
