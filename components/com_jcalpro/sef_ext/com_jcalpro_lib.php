 ...
 **********************************************
 ...
 $Id: com_jcalpro_lib.php 568 2010-01-17 22:52:17Z shumisha $
...
if (!function_exists( 'jclGetCategoryTitle')) {
   ...
   */
  function jclGetCategoryTitle( $cat_id, $option, $shLangName) {
    ...
    // fetch title from db if needed
    if (empty( $titles[$cat_id])) {
      // this particular title has not been found before, need to read from db
      $db = & JFactory::getDBO();
      $query = 'select ' . $db->nameQuote( 'cat_id')
      . ', ' . $db->nameQuote( 'cat_name')
      . ' from #__jcalpro2_categories'
      . ' where ' . $db->nameQuote( 'cat_id') . ' = ' . $db->Quote( $cat_id) . ';';
      $db->setQuery( $query);
      if (shTranslateUrl($option, $shLangName)) {
        $title = $db->loadObject();
      } else {
      ...
      // have the same title.
      $sefConfig = & shRouter::shGetConfig();
      $titles[$cat_id] = ( $sefConfig->jclInsertCategoryId ? $cat_id : '') . (empty( $title) ? '' : ' ' . $title->cat_name);
    }

    // return cached value
    return $titles[$cat_id];
  }
}
