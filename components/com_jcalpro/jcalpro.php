 ...
 **********************************************
 ...
 $Id: jcalpro.php 634 2010-03-30 11:49:34Z shumisha $
    ...
    while ($i < 7) {
      ...
      if (!empty( $events[$currentDay])) {
        ...
        while (list(,$event_row) = each($events[$currentDay])) {
          ...
          if ($CONFIG_EXT['popup_event_mode']) {
            //$non_sef_href = "index.php?option=" . $option . $Itemid_Querystring ."&extmode=view&extid=".$event->extid . '&amp;tmpl=component' . "&amp;date=$urlTargetDate" . (empty( $returnTo) ? '' : '&return_to=' . $returnTo);
            $non_sef_href = "index.php?option=" . $option . $Itemid_Querystring ."&extmode=view&extid=".$event->extid . '&amp;tmpl=component' . "&amp;date=$urlTargetDate";
            $link = 'href="'.JRoute::_($non_sef_href).'" class="jcal_modal" rel="{handler: \'iframe\'}" ';
          } else {
            //$sef_href = JRoute::_( $CONFIG_EXT['calendar_calling_page']."&extmode=view&extid=". $event->extid . "&amp;date=$urlTargetDate" . (empty( $returnTo) ? '' : '&return_to=' . $returnTo));
            $sef_href = JRoute::_( $CONFIG_EXT['calendar_calling_page']."&extmode=view&extid=". $event->extid . "&amp;date=$urlTargetDate");
            $link = "href=\"$sef_href\"";
          }
...
function print_categories_view() {
  ...
  global $CONFIG_EXT, $today, $lang_cats_view, $lang_system, $cat_id, $cat_list, $cal_id, $cal_list;
      ...
      foreach( $rows as $row) {
        if ( has_priv ( 'category' . $row->cat_id ) ) {
          if($CONFIG_EXT['archive']) {
            $query = "SELECT * FROM ".$CONFIG_EXT['TABLE_EVENTS'] . " WHERE cat = '$row->cat_id' AND approved = 1 $calendarQuery AND published = 1 ORDER BY title";
            ...
            $query = "SELECT * FROM ".$CONFIG_EXT['TABLE_EVENTS'] . " AS e WHERE (DATE_FORMAT(e.start_date,'%Y%m%d') >= $day_pattern) AND cat = '$row->cat_id'  $calendarQuery AND approved = 1 AND published = 1 ORDER BY title";
          }
          $db->setQuery($query);
          $db->query();

          $total_events = $db->getNumRows();
          $query = "SELECT count(`extid`) FROM ".$CONFIG_EXT['TABLE_EVENTS'] . " AS e WHERE (DATE_FORMAT(e.start_date,'%Y%m%d') > $day_pattern OR ( e.recur_type <> '' AND DATE_FORMAT(e.recur_until,'%Y%m%d') >= $day_pattern ) ) AND cat = $row->cat_id $calendarQuery AND approved = 1 and published = 1";
          $db->setQuery($query);
      ...
      if ($total_events) {
        foreach( $rows as $rawRow) {
          ...
          if ($CONFIG_EXT['popup_event_mode']) {
            ...
            $event_rows[$count]['link'] = 'href="' . $non_sef_href . '" class="jcal_modal" rel="{handler: \'iframe\'}" ';
          } else {
