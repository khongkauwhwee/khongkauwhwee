 ...
 **********************************************
 ...
 $Id: functions.inc.php 634 2010-03-30 11:49:34Z shumisha $
...
function jcHourToDisplayString( $hour, $minute = null, $force24Hours = null) {
  ...
  if($use24Hours) {
    ...
    // amp/pm
    if ($hour == 0) {
      $displayItem = '00'. $minuteDisplay .' - '. $lang_general['midnight'];
    } else if ($hour == 12) {
      $displayItem = '12' . $minuteDisplay . ' - '. $lang_general['noon'];
...
function display_event_form($target, $extmode, $form, $event_mode = '', $backLink = '') {
  ...
  if (!$user->guest) {
    ...
    $params['{SHOW_PRIVATE_EVENT_SUB_TEMPLATE}'] = '';
...
function get_events( $first_date, $last_date, $include_recurrent = JCL_SHOW_RECURRING_EVENTS_ALL, $show_overlapping_recurrences = false, $last_updated = false, $profiledUserId = 0) {
  ...
  global $CONFIG_EXT, $cal_id, $cat_id, $cat_list, $cal_list, $private_events_mode, $today, $mainframe;
  $db = &JFactory::getDBO();
  ...
  // read from db
  $query = "SELECT e.*, c.cat_id, c.cat_name, c.color, c.description as cat_desc from " . $CONFIG_EXT['TABLE_EVENTS'] . ' AS e'
...
function get_active_categories( $applyMenuRestrictions = false) {
  ...
  if (!empty( $cat_list)) {
    $where .= ' AND cat_id in (' . $cat_list . ')';
  }

  $query = "SELECT cat_id, cat_name, description AS cat_desc, color FROM " . $CONFIG_EXT['TABLE_CATEGORIES'] . $where . " ORDER BY cat_name";
  $db->setQuery($query);
