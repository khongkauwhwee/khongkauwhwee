 ...
 **********************************************
 *
 * $Id: latest.inc.php 595 2010-03-18 19:07:02Z shumisha $
 *
...
$EXTCAL_CONFIG['categories_list'] = $params->def('categories','');
$EXTCAL_CONFIG['calendars_list'] = $params->def('calendars_list','');
...
$EXTCAL_CONFIG['events_list'] = $params->def( 'events_list', '');
...
if ($EXTCAL_CONFIG['number_of_events_to_list_upcoming'] || $forcedEventId) {
  ...
  $count=0;
  $recIdsList = array();
  while(  ( count($rowsUpcoming) < $EXTCAL_CONFIG['number_of_events_to_list_upcoming'] || $forcedEventId) && ( $count < $reccount ) ) {
...
if ($EXTCAL_CONFIG['number_of_events_to_list_recent'] || $forcedEventId) {
  ...
  $count = 0;
  $recIdsList = array();
  while(  ( count($rowsRecent) < $EXTCAL_CONFIG['number_of_events_to_list_recent']  || $forcedEventId) &&
  ( $count < $reccount ) ) {
...
if ( ((count($rowsUpcoming) + count($rowsRecent)) > 0) || $EXTCAL_CONFIG['display_if_no_events'] ) {  // IF: Don't display if there are no events and the parameter is set not to
  ...
  if ( count($rowsUpcoming) ) {
    ...
    foreach ( $rowsUpcoming as $thisEvent ) { // For each upcoming event we do:
      ...
      $ext_event_link_URL = JRoute::_( 'index.php?option=com_jcalpro&amp;Itemid=' . $EXTCAL_CONFIG['Itemid'] . '&amp;extmode=view&amp;extid='. $thisEvent->extid .($CONFIG_EXT['popup_event_mode'] ? '&amp;tmpl=component': ''));
      ...
      $module_output_array[$extcounter] .= '<a href="' . $ext_event_link_URL . '" '.($CONFIG_EXT['popup_event_mode'] ? 'class="jcal_modal" rel="{handler: \'iframe\'}" ': '').'>' . mf_shorten_with_ellipsis($thisEvent->title,$EXTCAL_CONFIG['title_max_length']);
      ...
      $calCatString = $EXTCAL_CONFIG['show_calendar'] ? '(<small>' . stripslashes( $thisEvent->cal_name) . '</small>) ' : '';
      $calCatString .= $EXTCAL_CONFIG['show_category'] ? '(<small>' . stripslashes( $thisEvent->cat_name) . '</small>) ' : '';
