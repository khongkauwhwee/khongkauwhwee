 ...
 **********************************************
 ...
 $Id: com_jcalpro.php 599 2010-03-19 17:35:30Z shumisha $
...
switch ($extmode) {
  ...
  case 'cats' :
    ...
    break;
  case 'cat' :
    $catTitle = jclGetCategoryTitle( $cat_id, $option, $shLangName);
    if (!empty( $catTitle)) {
  ...
  case 'view' :
    ...
    if (!empty( $eventData)) {
      ...
      // set category
      $eventCat = jclGetCategoryTitle( $eventData->cat, $option, $shLangName);
      if (!empty( $eventCat)) {
        $title[] = $eventCat;
      }
      // start date
