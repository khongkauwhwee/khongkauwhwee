...
$theme_info = array (
    ...
    if ($isPrint) {
      ...
      $todayclr = '#ffffff'; # color today
      $sundayclr = '#EFEFEF'; # color calendarsunday
      $weekdayclr = '#F8F8F8'; # color calendarweekday
...
<!-- BEGIN print_view -->
                                        <td class="buttontext" align="center" valign="middle" nowrap='nowrap'>
                                          <a href="{PRINT_TGT}" onClick="jclPrintWindow=window.open('{PRINT_TGT}','jclPrintWindow','toolbar=no,location=no,directories=no,status=no,menubar=yes,scrollbars=yes,resizable=yes,width=800,height=600'); return false;" target="_blank">
                                          <img src="{URL}themes/{$CONFIG_EXT['theme']}/images/icon-print-toolbar.gif" border="0" alt="{PRINT_LNK}" /><br />
                                          {PRINT_LNK}</a>
                                        </td>
<!-- END print_view -->
                                </tr>
...
EOT;
    ...
    $template_add_event_form = <<<EOT
                 ...
                 {DO_NOT_SHOW_PRIVATE_EVENT_SUB_TEMPLATE}
...
EOT;
    ...
    <!-- BEGIN admin_row -->
                ...
                <tr>
                        ...
                        {SHOW_PRIVATE_EVENT_SUB_TEMPLATE}
                </tr>
<!-- END admin_row -->
<!-- BEGIN captcha_row -->
  <tr>
...
EOT;
    ...
    $template_add_event_form_do_not_show_private = '
                <input name="{PRIVATE_LABEL}" type="hidden" value="{PRIVATE_VAL}" />';
...
EOT;
    ...
    $template_cat_events_list = <<<EOT
                 <!-- BEGIN info1_row -->
                 <tr class="tablec">
                 <td class="tablec" colspan="2" align="left" nowrap='nowrap'><span class="atomic">{CATEGORY_DESC}</span></td>
                </tr>
<!-- BEGIN noevents_row -->
                <tr class="tableb">
...
EOT;
    ...
    $template_cat_legend = <<<EOT
                 ...
                 <!-- END start_col_row -->
                 <!-- BEGIN today_row -->
                 <td bgcolor="{COLOR}" class="legend-color-borders">
...
EOT;
    ...
    function theme_cat_events_list(&$results, $cat_info, $stats) {
      ...
      $document->setTitle( $cat_info['cat_name'] . ' | ' . $currentMenuItem->name);
    ...
    function theme_view_event(&$event, $is_popup = false, $feedsLink = '') {
      ...
      // set the page title
      $document = & JFactory::getDocument();
      $document->setTitle( $event->title . ' | ' . $event->catName);
