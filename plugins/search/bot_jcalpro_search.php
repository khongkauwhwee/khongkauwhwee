 ...
 * Search Mambot
 *
 * $Id: bot_jcalpro_search.php 466 2009-09-27 15:11:56Z shumisha $
 *
...
function plgSearchCalendar( $text, $phrase='', $ordering='', $areas=null ) {
  ...
  foreach( $data as $item )
  {
    ...
    // (correctly) 'Public Fronted'. However, that will break stuff with users upgrading from past versions
    // as events already stored in DB will not work. Hence the bad fix. Could have used ucwords, but not reliable
    // with utf-8
    if (trim($item->level) == 'public frontend') {
      $item->level = 'Public Frontend';
    }
