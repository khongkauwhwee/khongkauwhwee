<?php
/**
* pPGallery (0.815_RC4d) Content Plugin for Joomla! 1.5+
...
//ini_set ('error_reporting', E_ALL);
  ...
  if (preg_match_all($regexp, $row->text, $matches, PREG_PATTERN_ORDER) > 0) {
          ...
          $content = "<span class=\"pp_gallery\">\n";
            ...
            for($a = 0; $a < $imgcount; $a++) {
              ...
              if (substr(strtolower($imageset[$a]->img_file),-3) == 'jpg') {
                $thb_file = 'cache'."/".$imageset[$a]->img_path."/".$thb_w."x".$thb_h."_q".$quality_jpg."_t_".$imageset[$a]->img_file;
              }
              if (substr(strtolower($imageset[$a]->img_file),-3) == 'png') {
                $thb_file = 'cache'."/".$imageset[$a]->img_path."/".$thb_w."x".$thb_h."_q".$quality_png."_t_".$imageset[$a]->img_file;
              }
              if (substr(strtolower($imageset[$a]->img_file),-3) == 'gif') {
                $thb_file = 'cache'."/".$imageset[$a]->img_path."/".$thb_w."x".$thb_h."_t_".$imageset[$a]->img_file;
              }
    ...
    if (!defined('onlyonce')){
      ...
      $head = "<script type='text/javascript' charset='utf-8'> //rc5 \n";
...
function plgContentPPGalleryGetAttr($tag, $attr) {
  ...
  if (isset($attr_value[1])) {
    if (strtolower($attr) == 'src') $attr_value[1] = ltrim($attr_value[1], DS);
    return $attr_value[1];
  }
  return '';
}
